...
export default function BunFullstackDemo() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex flex-col items-center justify-center p-8 space-y-8">
      <div className="bg-white shadow-2xl rounded-3xl p-8 w-full max-w-4xl transform transition-all hover:scale-105 hover:shadow-4xl duration-300">
        ...
        <div className="grid grid-cols-2 gap-8">
          <div className="bg-blue-50 rounded-xl p-6 border-2 border-blue-200 hover:border-blue-400 transition-all">
            ...
            <pre className="bg-blue-100 rounded-lg p-4 text-sm text-blue-900">
              {`Bun.serve({
  fetch(req) {
    if (req.url.endsWith('/api/users')) {
      return Response.json(users);
            ...
            {loading ? (
              ...
              <ul className="space-y-2">
                {users.map((user, index) => (
                  <li 
                    key={index} 
                    className="bg-purple-100 px-4 py-2 rounded-lg text-purple-800 hover:bg-purple-200 transition-colors"
                  >
        ...
        <div className="mt-8 text-center">
          <div className="inline-block bg-green-50 border-2 border-green-300 px-6 py-3 rounded-full text-green-700 font-semibold hover:bg-green-100 transition-all">
            Full-stack Simplicity with Bun 🚀
          </div>
        </div>
      </div>
    </div>
